...
🧠 Hypothesis
A deep neural network trained on bit-sliced ML-KEM ciphertexts can classify a property of the corresponding shared secret at accuracy statistically above the 50% random baseline, indicating potential leakage.

❌ Hypothesis Outcome
The hypothesis was not supported.
Across all experiments, model performance remained statistically indistinguishable from random guessing (~50%), indicating no learnable structure in ML-KEM ciphertexts under this threat model.
...
Goal:
Predict a binary property of the shared secret using only ciphertext.
...
1D ResNet on raw ciphertext bits
